...
export const MessagesView: React.FC<MessagesViewProps> = ({
  ...
  const scrollConversationsUp = () => {
    if (conversationsContainerRef.current) {
      conversationsContainerRef.current.scrollBy({ top: -180, behavior: 'smooth' });
    }
  };

  const scrollConversationsDown = () => {
    if (conversationsContainerRef.current) {
      conversationsContainerRef.current.scrollBy({ top: 180, behavior: 'smooth' });
    }
  };
  ...
  const scrollMessagesUp = () => {
    if (messagesContainerRef.current) {
      messagesContainerRef.current.scrollBy({ top: -250, behavior: 'smooth' });
    }
  };
    ...
    <div className="w-full max-w-7xl mx-auto h-[calc(100vh-80px)] min-h-[560px] max-h-[820px] bg-[#1a1a1a] rounded-3xl border border-neutral-800 shadow-2xl overflow-hidden grid grid-cols-1 md:grid-cols-12 relative">
      ...
      <div className="md:col-span-4 border-r border-neutral-800 flex flex-col bg-[#141414]">
        ...
        <div className="flex-1 flex overflow-hidden relative">
          ...
          {/* Left Side Custom Scroll Control Bar (▲ / ▼) */}
          <div className="w-5 bg-[#1a1a1a] border-l border-neutral-800 flex flex-col items-center justify-between py-1 shrink-0 select-none z-10">
            <button
              type="button"
              onClick={scrollConversationsUp}
              className="p-1 hover:bg-neutral-800 active:bg-emerald-600 text-neutral-300 hover:text-emerald-400 rounded transition"
              title="Scroll Up Conversations"
            >
              <ArrowUp className="w-3 h-3" />
            </button>
            <div className="flex-1 w-1 my-1 bg-neutral-700/60 rounded-full hover:bg-emerald-500 transition cursor-pointer" />
            <button
              type="button"
              onClick={scrollConversationsDown}
              className="p-1 hover:bg-neutral-800 active:bg-emerald-600 text-neutral-300 hover:text-emerald-400 rounded transition"
              title="Scroll Down Conversations"
            >
      ...
      {activeConversation ? (
        <div className="md:col-span-8 flex flex-col h-full bg-[#181818] relative">
          ...
          <div className="flex-1 flex overflow-hidden relative">
            ...
            {/* Right Side Custom Scroll Control Bar (▲ / ▼) */}
            <div className="w-5 bg-[#141414] border-l border-neutral-800 flex flex-col items-center justify-between py-1 shrink-0 select-none z-10">
              <button
                type="button"
                onClick={scrollMessagesUp}
                className="p-1 hover:bg-neutral-800 active:bg-emerald-600 text-neutral-300 hover:text-emerald-400 rounded transition"
                title="Scroll Up Messages"
              >
